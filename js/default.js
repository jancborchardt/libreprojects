	...
	$.each(lp.categories, function(cidx, category) {
		...
		$.each(lp.projects, function(pidx, project) {
			if (category.id == project.category) {
				$('<li />').html('<a href="' + project.address + '"><img src="logos/' + project.id + '.png" alt="" /><span class="project"><strong>' + project.name + '</strong><span class="translatable">' + project.description + '</span></span></a></li>').
					    appendTo($ul);
			}
		} );
	} );

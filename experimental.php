...
<h1><a href="http://libreprojects.net/"><img src="favicon-touch.png" /><?=_("Libre Projects")?></a></h1>
<?php try { $count = $db->prepare('SELECT COUNT(*) AS projectcount FROM projects;'); $count->execute(); } catch (Exception $e) { die($e); } ?>
<p><?php echo $count->fetchObject()->projectcount ?> <?=_("free web services & alternatives")?></p>
...
<form action='/' id='search'><input class='searchfield' name='query' placeholder='<?=_("Search Libre Projects")?>' result='5' type='search'></form> 
